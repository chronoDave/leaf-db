...
test('[delete] should delete all data on empty query', async t => {
  ...
  try {
    ...
    for (let i = 0, v = Object.values(db._store._map); i < v.length; i += 1) {
      t.true(v[i].$deleted);
    }
  } catch (err) {
    t.fail(err);
...
test('[delete] should delete docs if matches are found (simple)', async t => {
  ...
  try {
    ...
    t.true(db._store._map.get('key_1').$deleted);
  } catch (err) {
    t.fail(err);
  }
...
test('[delete] should replace docs if matches are found (nested)', async t => {
  ...
  try {
    ...
    t.strictEqual(docs, 1);
    t.true(db._store._map.get('key_5').$deleted);
  } catch (err) {
    t.fail(err);
  }
...
test('[delete] should replace docs if matches are found (complex)', async t => {
  ...
  try {
    ...
    t.strictEqual(docs, 1);
    t.true(db._store._map.get('key_4').$deleted);
  } catch (err) {
    t.fail(err);
  }
